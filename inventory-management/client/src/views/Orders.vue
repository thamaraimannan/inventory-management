<template>
  <div class="orders">
    ...
    <div v-else>
      <!-- Submitted restocking orders: displayed only when at least one exists -->
      <div v-if="restockingOrders.length > 0" class="card submitted-orders-card">
        <div class="card-header">
          ...
          <span class="badge info">{{ t('restocking.status.submitted') }}</span>
        </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>{{ t('restocking.table.budget') }}</th>
            ...
            <tbody>
              <tr v-for="order in restockingOrders" :key="order.id">
                ...
                <td><span class="lead-time-badge">{{ order.lead_time_days }} {{ t('restocking.table.days') }}</span></td>
                <td>
                  <details class="items-details">
                    <summary>{{ t('orders.itemsCount', { count: order.items.length }) }}</summary>
                    <div class="items-dropdown">
                      <div v-for="(item, idx) in order.items" :key="idx" class="item-entry">
                        <span>{{ item.item_name }} ({{ item.item_sku }})</span>
                        <span class="item-meta">{{ t('orders.quantity') }}: {{ item.restock_quantity }} @ ${{ item.unit_cost }}</span>
                      </div>
                    </div>
                  </details>
                </td>
                ...
                <td><span class="badge info">{{ t('restocking.status.submitted') }}</span></td>
              </tr>
            </tbody>
          </table>
        </div>
